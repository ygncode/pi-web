...
The most important doc for frontend work is **`docs/dev/templates-vs-web.md`** — it explains the unified rendering where `web/` provides the live Vite app, and `internal/ui/embedded/` provides the Go-embedded shells. The static export snapshot bundle is built by Vite from `web/src/export/export-entry.js`, which **reuses the live session modules** in `web/src/session/` — there is no longer a hand-maintained parallel copy.
...
| `internal/ui/embedded/` | Go-embedded HTML shells for index/session pages |
...
- `internal/ui/session_page.go` — **Live session page** rendering (`internal/ui/embedded/session.html`, chat composer)
...
| HTML shell | `internal/ui/embedded/session.html` (`IsLive: true`) | `internal/ui/embedded/session.html` (`IsLive: false`) |
| JS source | `web/src/session/` (Vite) | `web/src/export/export-entry.js` (reuses `web/src/session/`), built → `internal/ui/embedded/export/export.js` + `vendor/` |
| CSS | `internal/ui/embedded/styles/session.css` | `internal/ui/embedded/styles/session.css` |
| Chat composer | Yes (`internal/ui/embedded/chat_composer.html`) | No |
| Action buttons | Yes (baked into `internal/ui/embedded/session.html`) | No |
...
1. **Live and export use a unified template.** `internal/ui/embedded/session.html` serves both the live app and Gist snapshots. Do not split them.

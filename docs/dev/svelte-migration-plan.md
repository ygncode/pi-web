...
| 🔶 next (largest) | **Phase 2 remainder: `session-entry-renderer.js` (638 LOC) → `<SessionContent>`/`<SessionEntry>`.** This is the message pane and the biggest single module: ~10 entry types, markdown, tool calls, images, code highlight, copy/fork/label buttons, collapsible tool output, token usage. It also replaces the navigator's imperative `#messages` build (active-path render, entry cache, toggle-state application, scroll/highlight, live-append) with a reactive `{#each model.activePath}`. Do it as its own carefully-verified step. |
